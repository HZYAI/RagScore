...
### Added
- **MCP `evaluate_rag` tool: `diagnose` + `context_fields` params** — AI assistants can now trigger failure diagnosis and specify context field names
- **CLI `--help` updated** — added `--diagnose` and `--context-fields` examples and "Diagnose mode" section
...
### Security
- **Fixed arbitrary file write in MCP server** — `get_corrections(output_path)` and `quick_test_rag(save_golden)` now validate paths via `resolve()` + `is_relative_to()`, restricting writes to `config.OUTPUT_DIR`
